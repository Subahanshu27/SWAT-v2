...
export function preflightReason(category: ErrorCategory): string {
  switch (category) {
    ...
    case 'community_input_missing':
      return 'Referenced file not found — re-upload (#inputs or #community_inputs) in Floyo, then Run + Publish.';
    case 'invalid_prompt_files':
      return 'Prompt files missing from storage — fix paths in Floyo editor, then Run + Publish.';
